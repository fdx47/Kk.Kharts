...
/// <summary>
/// Handler para o comando /usersstats - visão detalhada de uso.
/// </summary>
public sealed class UsersStatsCommandHandler(
    IServiceScopeFactory scopeFactory,
    ITelegramService telegram) : ITelegramCommandHandler
{
    public string Command => TelegramConstants.Commands.UsersStats;
    public string Description => "Statistiques détaillées d'utilisation";
    ...
    public async Task HandleAsync(Message message, CancellationToken ct = default)
    {
        ...
        var response = $"""
            {TelegramConstants.Emojis.Chart} <b>Statistiques d'utilisation</b>
        ...
        var keyboard = new InlineKeyboardMarkup(new[]
        {
            new[]
            {
                InlineKeyboardButton.WithCallbackData("🔄 Actualiser", "refresh:usersstats"),
                InlineKeyboardButton.WithCallbackData($"{TelegramConstants.Emojis.Device} Capteurs", "menu:devices"),
            },
            new[]
            {
                InlineKeyboardButton.WithCallbackData($"{TelegramConstants.Emojis.Warning} Voir Hors ligne", "filter:offline"),
                InlineKeyboardButton.WithCallbackData("🪫 Voir Batterie Faible", "filter:lowbattery"),
            },
            new[]
            {
